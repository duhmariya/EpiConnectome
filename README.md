...
│
├── pipeline/
│   ├── 01_edf_converter.py          # EDF → TXT extraction with annotation timestamps
│   ├── 02_preprocessing_wpli.py     # Bandpass · ICA · Bad channels · Channel-level wPLI
│   ├── 05_dspm_connectivity.py      # dSPM source localization · 18 ROIs · Source wPLI atlas
│   └── 06_gatv2_classification_v4.py # GATv2 classifier · K-Fold · LOSO · SVM baseline
│
├── results/
│   ├── SUMMARY_FIGURE.png           # Main classification results figure
│   ├── brain_topo_alpha.png         # Alpha attention topography
│   ├── brain_topo_beta.png          # Beta attention topography
│   ├── brain_topo_theta.png         # Theta attention topography
│   ├── subject_severity_heatmap.png # Patient-level severity overview
│   ├── GATv2_metrics_v4.xlsx        # Full metrics table (all bands, K-Fold + LOSO)
│   └── subject_severity_report.xlsx # Per-seizure severity labels
│
└── data/
...
```
Raw EDF Files  →  EDF→TXT Conversion  →  Preprocessing (4–40 Hz, ICA)
     ↓
Channel-Level wPLI (Script 02)  →  16×16 matrices, θ/α/β bands
     ↓
dSPM Source Localization (Script 05)  →  18 ROIs (HCPMMP1 parcellation)
     ↓
Source-Level wPLI (Script 05)  →  18×18 connectivity atlas
     ↓
GATv2 Classification (Script 06)  →  High/Low severity + attention maps
```
...
```bash
# Extract seizure segments from EDF files
python pipeline/01_edf_converter.py

# Preprocess and compute channel-level wPLI
python pipeline/02_preprocessing_wpli.py

# Source localization and source-level wPLI atlas
python pipeline/05_dspm_connectivity.py

# GATv2 classification (all results, figures, metrics)
python pipeline/06_gatv2_classification_v4.py
```

All outputs are saved to the directory specified by `OUTPUT_DIR`. Expected runtime for `06_gatv2_classification_v4.py`: ~20–40 minutes on CPU, ~5–10 minutes on GPU.
...
| `GATv2_metrics_v4.xlsx` | All metrics (K-Fold + LOSO + SVM baseline + Summary) |
