...
![GATv2 classification performance and permutation test significance across theta, alpha, and beta bands](results/GATv2_Results_v4/SUMMARY_FIGURE.png)
*Figure 1. GATv2 classification performance (left) and permutation test significance (right) across all three frequency bands.*

---

## 🔍 Interim Results & Sanity Checks

This section documents what we found at each pipeline stage — to help readers interpret the results and know what to expect when replicating, as well as to check our findings against known neuroscience.

### 1. Methodological Sanity Check — Volume Conduction Removal
...
> 📖 *Replicates: Vinck et al. (2011, NeuroImage) — wPLI at source level is systematically lower than sensor-level coherence due to removal of zero-lag interactions.*

---

### 2. Frequency Band Dominance
...
> 📖 *Replicates: Mormann et al. (2000, Clinical Neurophysiology) — theta-band phase synchronization is a key feature of seizure dynamics in TLE patients.*

> 📖 *Supported by: Buzsáki & Moser (2013, Nature Neuroscience) — theta oscillations coordinate hippocampal-cortical communication.*

---

### 3. Conceptual Sanity Check — Is the High vs Low Difference Real?
...
- Observed mean |High − Low| wPLI ≈ **0.22–0.28** across all bands
- Maximum across all 1000 null shuffles ≈ **0.16–0.18**
- The observed effect **exceeds the entire null distribution**, not just the 95th percentile

High-severity seizures show **global cortical hyperconnectivity** — nearly uniform elevated wPLI across all 18×18 ROI pairs. Low-severity seizures show patchy, lower-magnitude patterns. This global hyperconnectivity during severe seizures is consistent with the concept of pathological hypersynchrony.

> 📖 *Supported by: Jiruska et al. (2013, Journal of Physiology) — seizure generation involves pathological hypersynchrony of cortical networks.*

![Permutation test for the alpha band showing the observed wPLI difference far exceeding the null distribution](results/GATv2_Results_v4/permutation_alpha.png)
*Figure 2. Permutation test (alpha band) — observed difference (orange line) sits entirely outside the null distribution from 1000 shuffles.*

![Channel connectivity heatmaps comparing Low severity, High severity, and the difference between them in the alpha band](results/GATv2_Results_v4/heatmap_alpha.png)
*Figure 3. Source-level wPLI connectivity (alpha band) — Low severity, High severity, and the difference (High − Low).*

→ Full set: `results/GATv2_Results_v4/permutation_{band}.png` and `heatmap_{band}.png` for theta, alpha, beta

---

### 4. Empirical Sanity Check — Subject-Level Consistency
...
Cross-band consistency is a strong empirical sanity check — if connectivity severity were noise, you would not expect the same patients to rank high across three independent band analyses.

![Subject-level seizure connectivity severity heatmap showing mean wPLI per patient across alpha, beta, and theta bands](results/GATv2_Results_v4/subject_severity_heatmap.png)
*Figure 4. Subject-level mean wPLI across all three bands — PN05 and PN07 consistently rank highest.*

→ Full file: `results/GATv2_Results_v4/subject_severity_heatmap.png`

---

### 5. Attention Map Findings vs Known Neuroscience
...
The **medial temporal lobe (ROI9, hippocampal region)** appeared consistently across bands — consistent with its established role as the primary seizure onset zone in temporal lobe epilepsy (Engel, 2001).
...
| Alpha | Beta | Theta |
|:---:|:---:|:---:|
| ![Alpha band attention topography](results/GATv2_Results_v4/brain_topo_alpha.png) | ![Beta band attention topography](results/GATv2_Results_v4/brain_topo_beta.png) | ![Theta band attention topography](results/GATv2_Results_v4/brain_topo_theta.png) |

*Figure 5. GATv2 attention weights mapped onto brain topography, by frequency band.*

---

### 6. Does Graph Structure Actually Help? GATv2 vs SVM
...
Both models achieve near-identical AUC. This is the **expected and correct result** — the wPLI connectivity signal itself is discriminative. GATv2's advantage over SVM is **interpretability**: attention maps identify which ROI connections drive the classification, which SVM cannot provide.

![GATv2 vs SVM baseline comparison bar chart across F1, AUC, and balanced accuracy](results/GATv2_Results_v4/gatv2_vs_svm.png)
*Figure 6. GATv2 vs SVM baseline — F1, AUC, and balanced accuracy across all bands.*

---

### 7. What to Expect When Replicating
...
> Crone, N. E., et al. (2006). High-frequency gamma oscillations and human brain mapping with electrocorticography. *Progress in Brain Research*, 159, 275–295.

> Engel, J. (2001). Mesial temporal lobe epilepsy: what have we learned? *The Neuroscientist*, 7(4), 340–352.

> Jiruska, P., et al. (2013). Synchronization and desynchronization in epilepsy: controversies and hypotheses. *Journal of Physiology*, 591(4), 787–797.
...
> Mormann, F., et al. (2000). Mean phase coherence as a measure for phase synchronization and its application to the EEG of epilepsy patients. *Clinical Neurophysiology*, 111(12), 2218–2237.
...
### Why wPLI?
Standard coherence is inflated by **volume conduction** — the same cortical source appearing in multiple scalp electrodes. wPLI discards zero-lag interactions, retaining only true brain-to-brain connectivity (Vinck et al., 2011).
...
*EpiConnectome · Brainhack School 2026 · github.com/duhmariya/EpiConnectome*
